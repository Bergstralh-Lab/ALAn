# ALAn
This image analysis package is first introduced in "Competition Between Cell-Cell and Cell-Substrate Adhesion Determines Epithelial Monolayer Architecture in Culture"

This read-me is associated with the ALAn image analysis tool. The analysis tool is written as a set of functions in python, which we run in a jupyter notebook. This read-me will include a list of each function, with all inputs and outputs as well as a brief mention of where things will be saved or requirements for locations to read in files if necessary. ALAn is to be used with 4D confocal stacks (xyz, multichannel images saved with a 512 by 512 ) in conjunction with a spreadsheet of segmented nuclear positions and sizes (xyz positions, volume in µm3­, and reference positions). 
